use std::error::Error;
use gpio_cdev::{Chip, LineRequestFlags, LineHandle, Error as GpioError};
...
impl DriveSystem {
  pub fn new() -> Result<Self, Box<dyn Error>> {
    Ok(Self{
      pin_left_forward: get_line_handle(LEFT_FORWARD)?,
      pin_left_reverse: get_line_handle(LEFT_REVERSE)?,
      pin_right_forward: get_line_handle(RIGHT_FORWARD)?,
      pin_right_reverse: get_line_handle(RIGHT_REVERSE)?,
...
fn get_line_handle(gpio_number: u32) -> Result<LineHandle, GpioError>{
    let mut chip = Chip::new("/dev/gpiochip0")?;
    let line = chip.get_line(gpio_number)?;
